...
void SnakeGame_t::getEmptyOrBorderCoordinatesRandomly(GameObjectCoordinates_t& coordinates)
{
    ...
    } while (character != ' ' and character != '-' and character != '|');
}
...
{
    return mvwinch(window, coordinates.first, coordinates.second);
}
...
{
    mvwaddch(window, gameObject.getCoordinates().first, gameObject.getCoordinates().second, gameObject.getCharacter());
}
...
void SnakeGame_t::handleNextSnakePiece(SnakePiece_t nextPiece)
{
    ...
    if (!growthObjects.empty() and !poisonObjects.empty())
    {
        ...
        {
            case ' ':
                handlerForEmptyObject(std::move(nextPiece));
                return;

            case 'O':
                handlerForGrowthObject(std::move(nextPiece));
                return;

            case 'X':
                handlerForPoisonObject(std::move(nextPiece));
                return;

            case '?':
                handlerForGateObjects(std::move(nextPiece));
                return;
...
void SnakeGame_t::handlerForGateObjects(SnakePiece_t nextPiece)
{
    ...
    {
        GameObjectCharacter_t character = 0;

        while (character != ' ')
        {
            if (snakeObject->getHeadingDirection() == HeadingDirection_t::right and ((character = getGameObjectCharacterFromWindow(mainScreen->getGameWindow(), { nextPiece.getCoordinates().first, nextPiece.getCoordinates().second + 1 })) != ' '))
                snakeObject->setHeadingDirection(HeadingDirection_t::down);
            if (snakeObject->getHeadingDirection() == HeadingDirection_t::left and ((character = getGameObjectCharacterFromWindow(mainScreen->getGameWindow(), { nextPiece.getCoordinates().first, nextPiece.getCoordinates().second - 1 })) != ' '))
                snakeObject->setHeadingDirection(HeadingDirection_t::up);
            if (snakeObject->getHeadingDirection() == HeadingDirection_t::down and ((character = getGameObjectCharacterFromWindow(mainScreen->getGameWindow(), { nextPiece.getCoordinates().first + 1, nextPiece.getCoordinates().second })) != ' '))
                snakeObject->setHeadingDirection(HeadingDirection_t::left);
            if (snakeObject->getHeadingDirection() == HeadingDirection_t::up and ((character = getGameObjectCharacterFromWindow(mainScreen->getGameWindow(), { nextPiece.getCoordinates().first - 1, nextPiece.getCoordinates().second })) != ' '))
                snakeObject->setHeadingDirection(HeadingDirection_t::right);
        }
...
void SnakeGame_t::createGrowthObject(std::unique_ptr<GrowthObject_t>& growthObject)
{
    ...
    addGameObjectCharacterToWindow(mainScreen->getGameWindow(), *static_cast<GameObject_t*>(growthObject.get()));
}
...
void SnakeGame_t::createPoisonObject(std::unique_ptr<PoisonObject_t>& poisonObject)
{
    ...
    addGameObjectCharacterToWindow(mainScreen->getGameWindow(), *static_cast<GameObject_t*>(poisonObject.get()));
}
...
void SnakeGame_t::initializeStageMissions()
{
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> distMissionIndex(0, static_cast<int>(stageMissionKeys.size()));
    std::uniform_int_distribution<int> distMissionValue(5, 20);

    for (int i = 0; i < 4; i++)
        stageMissions[i] = { stageMissionKeys[distMissionIndex(gen)], distMissionValue(gen) };
}
