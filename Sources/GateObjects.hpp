...
public:
    ...
    explicit GateObjects_t(GameObjectCoordinates_t firstGateCoordinates, GameObjectCoordinates_t secondGateCoordinates) noexcept : gates(GatePiece_t(std::move(firstGateCoordinates)), GatePiece_t(std::move(secondGateCoordinates))) {}

    // This function will set value of first gate
    void setFirstGate(GatePiece_t firstGateInput) { gates.first = std::move(firstGateInput); }

    // This function will set value of second gate
    void setSecondGate(GatePiece_t secondGateInput) { gates.second = std::move(secondGateInput); }
